# MATH Kaggle Execution

This note prepares the next `pilot_only` Kaggle run for `MATH` on
`Qwen/Qwen2.5-1.5B-Instruct`.

## Goal

Run one end-to-end `train -> extract -> SVD` pilot for `MATH` using the
...
- `python training/train_grpo_task.py --task MATH --print-task-config`
- `python training/train_grpo_task.py --task MATH --smoke-test-prompt`
- `python runs/run_phase0_pipeline.py --date 2026-04-23 --task math --model-id Qwen/Qwen2.5-1.5B-Instruct --artifacts-root /tmp/ascent-artifacts --dry-run`
...
The `numeric` subset is used because it already exposes a numeric final answer
compatible with the current `final_number_exact_match` reward path.
...
- training output dir:
  `/kaggle/working/2026-04-23-phase0-math-qwen2.5-1.5b/`
- run dir:
  `runs/2026-04-23-phase0-math-qwen2.5-1.5b/`
...
python training/train_grpo_task.py \
  --task MATH \
  --model-id Qwen/Qwen2.5-1.5B-Instruct \
  --output-dir /kaggle/working/2026-04-23-phase0-math-qwen2.5-1.5b \
  --max-steps 50 \
  --scope pilot_only
```
...
python extraction/extract_registered_update_vector.py \
  --task MATH \
  --model-id Qwen/Qwen2.5-1.5B-Instruct \
  --adapter-path /kaggle/working/2026-04-23-phase0-math-qwen2.5-1.5b/adapter \
  --output-path /kaggle/working/2026-04-23-phase0-math-qwen2.5-1.5b/update_vector.npy
```
...
python analysis/pilot_svd_diagnostic.py \
  ...
  --model-id Qwen/Qwen2.5-1.5B-Instruct \
  --adapter-path /kaggle/working/2026-04-23-phase0-math-qwen2.5-1.5b/adapter \
  --output-path runs/2026-04-23-phase0-math-qwen2.5-1.5b/analysis_report.json
```

## Expected Outputs

Under `/kaggle/working/2026-04-23-phase0-math-qwen2.5-1.5b/`:
